...
<div class="super-theme-example">
	...
	<div id="itemEditWindow" class="easyui-window" title="My Window"
		...
		data-options="iconCls:'icon-save',modal:true,closed:'true',href:'${pageContext.request.contextPath}/item-edit'">
	</div>
</div>
			...
			handler : function() {
				...
				$.messager.confirm('确认', '您确认上架' + 'id为' + ids + '的商品吗？', function(r) {
					if(r) {
						...
						$.post("${pageContext.request.contextPath}/item/putUp",params1,function(data){
							if(data.status == 200){
								alert("上架成功！");
								$("#dgTbItem").datagrid("reload");
							}else{
			...
			handler : function() {
				...
				$.messager.confirm('确认', '您确认下架' + 'id为' + ids + '的商品吗？', function(r) {
					if(r) {
						...
						$.post("${pageContext.request.contextPath}/item/putDown",params2,function(data){
							if(data.status == 200){
								alert("下架成功！");
								$("#dgTbItem").datagrid("reload");
							}else{
